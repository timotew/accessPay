<?php
class Savvy_Accesspay_Helper_Data extends Mage_Core_Helper_Abstract
{
    /**
     * Log file name
     *
     * @var string
     */
    protected $_logFile = 'savvy_accesspay.log';
    ...
    const PAYMENT_ACCESSPAY_SYSTEM_PATH          ='payment/accesspay';
    ...
    public function getMerchantDetails()
    {
        ...
        $accessAccount = array(
            'merchant_id'          => trim(Mage::helper('core')->decrypt($this->getConfigData('merchant_id', 'payment_accesspay'))),
            'currency_code'          => strtoupper(trim($this->getConfigData('currency_code', 'payment_accesspay'))),
            'merchant_email'          => trim(Mage::helper('core')->decrypt($this->getConfigData('merchant_email', 'payment_accesspay')))
        );
    ...
    public function getConfigData($var, $type, $store = null)
    {
        ...
        switch ($type) {
            ...
            case 'payment_accesspay':
                $path = self::ACCESSPAY_SETTINGS_SYSTEM_PATH;
                break;
        }
